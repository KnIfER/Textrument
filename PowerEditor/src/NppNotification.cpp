...
void tweakTabBarCMShowOpenLnk(ContextMenu& tabPopupMenu, NativeLangSpeaker& nativeLangSpeaker, bool showOpenLnk) {
	if (tabPopupMenu.flag!=showOpenLnk) {
		...
		if(showOpenLnk) {
			InsertMenu( tabPopupMenu.getMenuHandle(), 13, MF_BYPOSITION,  IDM_FILE_OPEN_LINK, TEXT("Open Linked File / Directory"));
...
BOOL Notepad_plus::notify(SCNotification *notification)
{
	...
	switch (notification->nmhdr.code)
	{
		...
		case NM_RCLICK :
		{
			...
			_tabPopupMenu.enableItem(IDM_FILE_OPEN_DEFAULT_VIEWER, !isUntitled);

			_tabPopupMenu.enableItem(IDM_VIEW_GOTO_NEW_INSTANCE, !(isDirty||isUntitled&&(buf->docLength()>0||buf->isModified())));
